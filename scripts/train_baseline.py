...
class Baseline(nn.Module):
    ...
    def state_rollout(self, z_seq: torch.Tensor, dt: float = 1e-3, seq_len: int = None) -> torch.Tensor:
        ...
        if not seq_len:
            ...
            output = odeint(func=self.model, y0=z0, t=t)
    ...
    def forward(self, x: torch.Tensor,
        ...
        # predict
        z_pred = self.state_rollout(z_seq=z_rs)
        ...
        return xhat_recon, xhat_pred, z_rs, z_pred
...
def baseline_loss_fcn(x: torch.Tensor,
    ...
    x_input = x[:, :-1, ...]
    x_target = x[:, 1:, ...]
    z_enc_target = z_enc[:, 1:, ...]
    z_pred_target = z_pred[:, :-1, ...]
    
    
    loss_ae = ae_loss(x=x_input, xhat_recon=xhat_recon)
    loss_dyn = dynamics_loss(x=x_target, xhat_pred=xhat_pred)
    loss_latent = ((z_pred_target - z_enc_target)**2).sum()
...
def train_epoch(args, model, dataloader, optimizer):
    ...
    for i, x in enumerate(dataloader): 
        ...
        xhat_recon, xhat_pred, z_enc, z_pred = model(x_input.float(), dt=args.dt)
...
def plot_pred_output(args, model, test_dataloader):
    ...
    x_input = test_data[0, :-1,  ...].unsqueeze(0).to(args.device)
    x_target = test_data[0, 1:,  ...].unsqueeze(0).to(args.device)
    
    model.eval()
    with torch.no_grad():
        _, x_pred, _, _ = model(x_input.float(), dt=args.dt)
...
def plot_recon_output(args, model, test_dataloader):
    ...
    test_data = next(iter(test_dataloader))
    x = test_data[0, :-1, ...].unsqueeze(0).to(args.device)
    
    model.eval()
    with torch.no_grad():
        x_recon, _, _, _ = model(x.float(), dt=args.dt)
...
def main():
    ...
    decoder = DecoderNet()
    LSTM = torch.nn.LSTM(input_size=6, hidden_size=50, num_layers=3, batch_first=True)
    MLP = torch.nn.Linear(in_features=50, out_features=6)

    baseline = Baseline(encoder=encoder, decoder=decoder, baseline=LSTM, mlp=MLP)
    ...
    for p in baseline.mlp.parameters():
        num_params += np.prod(p.shape)
